...
const requestPool = ({log, timeout = 30000}) => {
    ...
    const int = setInterval(() => {
        log('trace', `Requests count: ${pool.size}`);
    }, 10000);
    const p = {
        add(message) {
...
const Server = (
    ...
    const o = {
        ...
        connected(client, request) {
            ...
            client.on('message', (m) => mOk(m, request, {connectionId}));
    ...
    class wss extends prev {
        constructor() {
            ...
            server = Server(
                ...
                {
                    log: (level, ...a) => logger[level](a),
                    mOk: (a, request, b) => this.clientFrom(a, request, b),
        ...
        async clientTo({id, params, meta: {connectionId: cId}}) {
            ...
            try {
                ...
                logger.error({on: 'write', error: serializeError(e.error || e)});
                throw e;
            }
        }
        async clientFrom(payload, request, meta) {
            let decoded;
            let response;
            let destination;
            try {
                // decode comes from codec
                ({intRoute: destination, ...decoded} = this.decode(payload));
                ...
                if (!destination) {
                    throw new Error('DestinationMissing');
                }
                const resp = this.ask({
                    id: requestCounter(),
                    method: destination,
                ...
                if (!decoded.id) {
                    (async () => {
                        try {
                            ...
                            logger.error({on: 'notification', error: serializeError(e), decoded});
                        }
                    })();
                    return;
                }
                ...
                logger.error({
                    error: serializeError(e.error || e),
                    meta,
                    decoded,
        ...
        async clientClose({connectionId}) {
            logger.warn(`closing client with id: ${connectionId}`);
            try {
                await this.ask({
                    id: requestCounter(),
                    method: `${this.destination()}.received.disconnected`,
                    params: {},
                    meta: {
                        ...initMeta(),
